...
- **「練習字」分頁**：輸入默書唔識嘅字（中文 1–4 字／英文生字），逐字撳掣重溫讀音；仲可以「匯出 PDF」整一張 A4 練習紙，**一行過排版**：`1. 認真 □□□□□□`（詞語 → 隔少少 → 格仔 → 隔少少 → 又格仔，寫幾多次就有幾組，唔夠位先轉行），中文用 1.3cm × 1.3cm 格仔、**英文用橫線**，全張統一揀「寫幾多次」1–5 次，列印時另存為 PDF。
