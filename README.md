...
- **「練習字」分頁**：輸入默書唔識嘅字（中文 1–4 字／英文生字），逐字撳掣重溫讀音；仲可以「匯出 PDF」整一張 A4 練習紙：每個字左邊有灰色示範字，中文用 1.3cm × 1.3cm 格仔、**英文用橫線**（每條線夠長度寫成個字），全張統一揀「寫幾多次」1–5 次（例如「認真」×2 → 2 行 × 2 格；`Sunday` ×3 → 3 條橫線），列印時另存為 PDF。
