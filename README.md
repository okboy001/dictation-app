...
- **自動分段**：用 **Space 或換行分隔**每項；**標點會保留**。若一項超過 **10 個字**（唔計標點），會先喺標點位置斷開，仍然過長就按詞語邊界揀最平均嘅切口，跟住句子意思拆。
- **逐項朗讀**：每一項都有播放掣，點文字亦可播放；朗讀時會讀出標點名（`，`→逗號、`。`→句號、`！`→感嘆號、`？`→問號、`、`→頓號、`；`→分號、`：`→冒號、`…`→省略號、`「」『』`→引號），方便小朋友寫返標點。
- **語言切換**：粵語（`zh-HK`，預設）／普通話（`zh-CN`，包括台灣國語 `zh-TW` 候選），另有「聲線」下拉列出裝置**全部**聲線，粵、普各自記憶。搵唔到相應聲線會提示安裝（macOS：系統設定 → 輔助使用 → 朗讀內容 → 系統聲音 → 管理聲音 → 下載「普通話（中國）」）。
- **語速切換**：下拉揀 `0.25x`（極慢）／`0.3x`（超慢）／`0.4x`（預設）／`0.5x`（慢）／`0.6x`／`0.75x`／`0.85x`／`1.0x`。
